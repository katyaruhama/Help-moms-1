...
Веб-сервис на русском языке для проекта помощи мамам после родов. Внутри есть лендинг, форма заявки и блок AI-анализа: сайт делает запрос к Apify Website Content Crawler, получает текст страницы, отправляет его на анализ в LLM через OpenRouter или Google AI Studio и показывает результат на странице.
...
- `vercel.json` — настройка длительности serverless-функции.
