...
function createDemoAnalysis({ targetUrl, question, maxPages }) {
  ...
  const pageLabel = maxPages === 1 ? '1 источник' : `${maxPages} страниц`;
  const analysis = [
    ...
    '',
    'Краткое резюме',
    `Сервис показывает, как будет выглядеть результат после реального прохода Apify по сайту, публичной странице или каналу. В боевом режиме crawler получит ${pageLabel}, передаст текст в LLM и вернёт вывод на эту же страницу.`,
    '',
    'Идеи для волонтёрского проекта',
    '- Чётко описывать маленькие задачи, чтобы человеку было легче согласиться помочь.',
    '- Показывать реальные форматы поддержки: еда, аптека, покупки, короткая бытовая помощь.',
    '- Держать тон спокойным: без давления на маму и без героизации помощника.',
    '',
    'Практические выводы',
    '- Собрать короткие сценарии помощи и вынести их в открытые заявки.',
    '- Добавить тексты про границы: что помощник делает, а чего не делает.',
    '- Использовать найденные идеи для FAQ, онбординга волонтёров и карточек заявок.',
    '',
    'Риски',
    '- Если API-ключи не заданы или закончились credits, реальный анализ не выполнится.',
    '- Если соцсеть или канал закрыты от crawler-ботов, Apify может вернуть мало текста.',
    '- Для отдельных платформ могут понадобиться специальные Apify actors, а не общий Website Content Crawler.',
    '',
    'Следующие шаги',
    '1. Выбрать 3-5 публичных источников о волонтёрстве и помощи мамам.',
    '2. Прогнать их через демо, затем через реальный Apify + LLM режим.',
    '3. Перенести лучшие идеи в заявки, FAQ и страницу для помощников.'
  ].join('\n');
...
function normalizeQuestion(value) {
  ...
  return question || 'Проанализируй источник о волонтёрстве или помощи мамам: выдели полезные идеи, форматы поддержки, риски, тон коммуникации и 3 практических шага для проекта «Мамам рядом».';
}
